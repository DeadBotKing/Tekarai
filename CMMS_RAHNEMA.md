...
## داشبورد نگهداری (مدیریتی)
مسیر: **نگهداری و تعمیرات ← داشبورد نگهداری** (`/app/maintenance/dashboard`).

- **کارت‌های آماری:** درخواست‌های باز (+ تعداد بحرانی)، PM سررسیدشده، نرخ تکمیل، میانگین
  زمان رفع عیب (MTTR).
- **فیلتر بازه‌ی زمانی:** ۷ / ۳۰ / ۹۰ روز / همه — همه‌ی کارت‌ها و نمودارها زنده بازمحاسبه
  می‌شوند.
- **نمودارها:**
  - روند درخواست‌ها در طول زمان (ثبت‌شده در برابر تکمیل‌شده).
  - روند میانگین زمان رفع عیب (MTTR) در هر بازه.
  - درخواست‌های باز به تفکیک وضعیت — **کلیک روی هر ستون** به فهرست فیلترشده می‌رود.
  - نرخ تکمیل (دونات).
  - درخواست‌های باز به تفکیک اولویت — مرتب‌شده صعودی بر اساس تعداد، با رنگ اختصاصی هر اولویت.
  - بار کاری واحدها — **کلیک روی هر ردیف** به فهرست فیلترشده‌ی همان واحد می‌رود.
- **خروجی:** دکمه‌ی «خروجی CSV» (سازگار با Excel فارسی، شامل BOM) و «چاپ / PDF»
  (با استایل چاپ اختصاصی که منو/نوار بالا را حذف می‌کند).
