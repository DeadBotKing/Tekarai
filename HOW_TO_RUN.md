...
## ⚠️ اگر ویندوز گفت «file is not digitally signed»

سیاست اجرای اسکریپت ویندوز جلوی فایل‌های `.ps1` را می‌گیرد. به‌جای آن از لانچرهای
`.cmd` استفاده کنید که این محدودیت را ندارند:

```powershell
.\fix_login.cmd            # عیب‌یابی و رفع مشکل ورود (migrate + ساخت ادمین)
.\run_dev.cmd -UseSqlite   # اجرای کل برنامه بدون SQL Server
.\run_dev.cmd              # اجرای کل برنامه با SQL Server / SQL Express
```

نکته: قبل از باز کردن فایل ZIP، روی خودِ فایل زیپ راست‌کلیک کنید →
Properties → گزینهٔ **Unblock** را تیک بزنید → OK. این کار قفل «دانلود از اینترنت»
را از همهٔ فایل‌های داخل آن برمی‌دارد.
