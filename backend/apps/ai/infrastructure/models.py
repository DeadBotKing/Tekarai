...
class AIProviderModel(BaseAiModel):
    ...
    configurationReference = models.CharField(max_length=255, blank=True)
...
class AIModelModel(BaseAiModel):
    provider = models.ForeignKey(AIProviderModel, on_delete=models.PROTECT, related_name="models")
    code = models.CharField(max_length=120)
    name = models.CharField(max_length=160)
    modelType = models.CharField(max_length=40, default="LLM")
...
class AIPromptVersionModel(BaseAiModel):
    prompt = models.ForeignKey(AIPromptModel, on_delete=models.PROTECT, related_name="versions")
    version = models.PositiveIntegerField()
    template = models.TextField()
    systemInstruction = models.TextField(blank=True)
    variables = models.JSONField(default=list)
    outputSchema = models.JSONField(default=dict, blank=True)
    modelConstraints = models.JSONField(default=dict, blank=True)
...
class AIRequestModel(BaseAiModel):
    capability = models.ForeignKey(AICapabilityModel, on_delete=models.PROTECT)
    requestedBy = models.UUIDField(null=True)
    requestType = models.CharField(max_length=40)
    sourceDomain = models.CharField(max_length=100, blank=True)
    sourceEntityType = models.CharField(max_length=100, blank=True)
    sourceEntityId = models.CharField(max_length=100, blank=True)
    priority = models.CharField(max_length=20, default="NORMAL")
    status = models.CharField(max_length=30, default="PENDING")
    correlationId = models.CharField(max_length=128, db_index=True)
    ...
    idempotencyKey = models.CharField(max_length=160, blank=True)

    class Meta:
        db_table = "aiRequests"
        indexes = [models.Index(fields=["tenantId", "status", "createdAt"])]


class AIResponseModel(BaseAiModel):
    request = models.OneToOneField(
        AIRequestModel, on_delete=models.PROTECT, related_name="response"
    )
    model = models.ForeignKey(AIModelModel, on_delete=models.PROTECT)
    status = models.CharField(max_length=30)
    content = models.TextField(blank=True)
    structuredData = models.JSONField(default=dict, blank=True)
    ...
    outputClassification = models.CharField(max_length=30, default="ADVISORY")
    promptVersion = models.ForeignKey(AIPromptVersionModel, null=True, on_delete=models.PROTECT)
    createdAt = models.DateTimeField(auto_now_add=True)
...
class AIUsageModel(BaseAiModel):
    request = models.OneToOneField(AIRequestModel, on_delete=models.PROTECT)
...
class AIFeedbackModel(BaseAiModel):
    request = models.ForeignKey(AIRequestModel, on_delete=models.PROTECT)
    response = models.ForeignKey(AIResponseModel, on_delete=models.PROTECT)
...
class AIKnowledgeChunkModel(BaseAiModel):
    item = models.ForeignKey(AIKnowledgeItemModel, on_delete=models.CASCADE, related_name="chunks")
...
class AIAuditRecordModel(BaseAiModel):
    ...
    resultClassification = models.CharField(max_length=30, blank=True)
    metadata = models.JSONField(default=dict)
...
class AIUsageAttemptModel(BaseAiModel):
    request = models.ForeignKey(
        AIRequestModel, on_delete=models.PROTECT, related_name="usageAttempts"
    )
...
class AIQuotaCounterModel(BaseAiModel):
    policy = models.ForeignKey(
        AIQuotaPolicyModel, on_delete=models.PROTECT, related_name="counters"
    )
...
class AIEvaluationResultModel(BaseAiModel):
    ...
    run = models.ForeignKey(AIEvaluationRunModel, on_delete=models.CASCADE, related_name="results")
    caseCode = models.CharField(max_length=80)
    verdict = models.CharField(max_length=10, default="PASS")
