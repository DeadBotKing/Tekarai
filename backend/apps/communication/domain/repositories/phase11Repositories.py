...
class MeetingRoomRepository(Protocol):
    def findByMeeting(self, tenantId: uuid.UUID, meetingId: uuid.UUID) -> MeetingRoom | None: ...
    def getById(self, tenantId: uuid.UUID, roomId: uuid.UUID) -> MeetingRoom | None: ...
    def save(self, room: MeetingRoom) -> None: ...
    def saveSession(self, session: MeetingSession) -> None: ...
    def listSessions(self, tenantId: uuid.UUID, meetingId: uuid.UUID) -> list[MeetingSession]: ...
...
class MeetingSummaryRepository(Protocol):
    ...
    def getById(self, tenantId: uuid.UUID, summaryId: uuid.UUID) -> MeetingSummary | None: ...
...
class ActionItemRepository(Protocol):
    ...
    def getById(self, tenantId: uuid.UUID, itemId: uuid.UUID) -> ActionItemCandidate | None: ...
    def listForMeeting(
        self, tenantId: uuid.UUID, meetingId: uuid.UUID
    ) -> list[ActionItemCandidate]: ...
    def listPending(self, tenantId: uuid.UUID, *, limit: int = 50) -> list[ActionItemCandidate]: ...


@runtime_checkable
class OfficialMessageRepository(Protocol):
    def save(self, message: OfficialMessage) -> None: ...
    def getById(self, tenantId: uuid.UUID, officialId: uuid.UUID) -> OfficialMessage | None: ...
    def list(
        self,
        tenantId: uuid.UUID,
        *,
...
class LegalHoldRepository(Protocol):
    ...
    def listActiveForTarget(self, tenantId: uuid.UUID, targetId: uuid.UUID) -> list[LegalHold]: ...
