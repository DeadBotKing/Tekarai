...
class RecipientDeliveryRepository(Protocol):
    ...
    def getById(self, tenantId: uuid.UUID, deliveryId: uuid.UUID) -> RecipientDelivery | None: ...
    ...
    def listRetryDue(self, now: datetime, *, limit: int = 100) -> list[RecipientDelivery]: ...

    def listDeadLetter(
        self, tenantId: uuid.UUID, *, limit: int = 100
    ) -> list[RecipientDelivery]: ...

    def listAttempts(self, tenantId: uuid.UUID, deliveryId: uuid.UUID) -> list[DeliveryAttempt]: ...
...
class NotificationRuleRepository(Protocol):
    ...
    def getById(self, tenantId: uuid.UUID, ruleId: uuid.UUID) -> NotificationRule | None: ...

    def listForEvent(self, tenantId: uuid.UUID, eventType: str) -> list[NotificationRule]: ...
