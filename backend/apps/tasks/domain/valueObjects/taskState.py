...
TASK_PRIORITIES = (TASK_PRIORITY_LOW, TASK_PRIORITY_NORMAL, TASK_PRIORITY_HIGH, TASK_PRIORITY_CRITICAL)
...
class TaskPriority(ValueObject):
    ...
    def __post_init__(self) -> None:
        if self.value not in TASK_PRIORITIES:
            raise ValidationFailedError("Invalid task priority.", fieldErrors={"priority": self.value})
