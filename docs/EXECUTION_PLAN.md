...
| 4 | اعلان‌ها برای کاربر واقعی | ارسال اعلان به platform-admin + مشاهده در UI | ⏸ در انتظار |
...
- **باگ PowerShell 5.1 در اجرای واقعی روی ویندوز کاربر پیدا و رفع شد:** در ویندوز PowerShell 5.1، درخواست GET که `ContentType` داشته باشد با `Cannot send a content-body with this verb-type` شکست می‌خورد. رفع: در `exercise_flows.ps1` مقدار `ContentType` فقط وقتی body ارسال می‌شود (POST) ست می‌شود، نه GET.
...
- [ ] گام ۴: ارسال اعلان واقعی به platform-admin و مشاهده در UI.
- [ ] گام ۵: پروفایل / MFA / کلید API / ممیزی (HTTP).
