...
### 5.6. اصلاح پس از Verification دستی در Windows

در اجرای دستی اولیه از مسیر `backend`، تست C به‌دلیل استفاده از مسیر نسبی
`backend/apps/...` با `FileNotFoundError` متوقف شد. این مشکل مربوط به test
fixture بود، نه Provider Contract. مسیر خواندن Source به مسیر absolute مبتنی بر
`Path(__file__).resolve().parents[2]` تغییر کرد تا تست هم از ریشهٔ Repository و
هم از `backend/` قابل اجرا باشد.

پس از اصلاح:

```text
cd backend
PYTHONPATH=. python -m unittest tests.unit.testPhase13ProviderPort -v
Ran 9 tests
OK
```
