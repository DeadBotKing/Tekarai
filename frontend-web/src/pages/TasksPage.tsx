...
const taskColumns = (t: ReturnType<typeof useLocalization>["t"], onOpen: (task: Task) => void): DataTableColumn<Task>[] => [
  ...
  { key: "dueDate", label: t("task.due"), accessor: (row) => row.dueDate, sortable: true, render: (row) => <span>{formatJalali(row.dueDate, { style: "short" })}</span> },
];
...
function TaskBoard({ tasks, onMove, onOpen }: { tasks: Task[]; onMove: (task: Task, status: Task["status"]) => void; onOpen: (task: Task) => void }): JSX.Element {
  const { t } = useLocalization();
  return <div className="kanban">{columns.map((column) => <section className="kanban-column" key={column.key}><header><h3>{column.label}</h3><span>{tasks.filter((task) => task.status === column.key).length}</span></header><div className="kanban-column__body">{tasks.filter((task) => task.status === column.key).map((task) => <button type="button" className="task-card" key={task.id} onClick={() => onOpen(task)}><div className="task-card__top"><Badge tone={task.priority === "critical" ? "danger" : task.priority === "high" ? "warning" : "neutral"}>{task.priority}</Badge><Icon name="more" size={15} /></div><strong>{task.title}</strong><span>{task.project}</span><div className="task-card__bottom"><Avatar name={task.assignee} size="sm" tone="purple" /><span>{formatJalali(task.dueDate, { style: "short" })}</span></div>{column.key !== "done" && <span className="task-card__advance" onClick={(event) => { event.stopPropagation(); onMove(task, column.key === "backlog" ? "inProgress" : column.key === "inProgress" ? "review" : "done"); }}>{t("common.next")} <Icon name="arrowRight" size={13} /></span>}</button>)}</div></section>)}</div>;
}
